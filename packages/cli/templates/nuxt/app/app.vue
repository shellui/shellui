<script setup lang="ts">
import { t } from './i18n';

const { theme, language } = useShellui();

const themeText = computed(() => {
  const current = theme.value as { displayName?: string; name?: string; mode?: string } | null;
  if (!current) return t(language.value, 'fallbackTheme');
  const name = current.displayName || current.name || 'shellui';
  return `${name} · ${current.mode}`;
});
</script>

<template>
  <main class="shellui-home">
    <p class="eyebrow">Shellui</p>
    <h1>{{ t(language, 'title') }}</h1>
    <p class="blurb">{{ t(language, 'blurb') }}</p>

    <dl class="meta">
      <div>
        <dt>{{ t(language, 'themeLabel') }}</dt>
        <dd>{{ themeText }}</dd>
      </div>
      <div>
        <dt>{{ t(language, 'languageLabel') }}</dt>
        <dd>{{ language || t(language, 'fallbackLanguage') }}</dd>
      </div>
    </dl>

    <p class="hint">{{ t(language, 'hint') }}</p>
  </main>
</template>

<style>
:root {
  --radius: 0.5rem;
  --background: #ffffff;
  --foreground: #0a0a0a;
  --primary: #171717;
  --primary-foreground: #fafafa;
  --muted: #f5f5f5;
  --muted-foreground: #737373;
  --border: #e5e5e5;
  --font-family: system-ui, -apple-system, 'Segoe UI', sans-serif;
  --heading-font-family: var(--font-family);
  --body-font-family: var(--font-family);
}

.dark {
  --background: #0a0a0a;
  --foreground: #fafafa;
  --primary: #fafafa;
  --primary-foreground: #171717;
  --muted: #262626;
  --muted-foreground: #a3a3a3;
  --border: #262626;
}

html,
body {
  margin: 0;
  min-height: 100%;
  background: var(--background);
  color: var(--foreground);
  font-family: var(--body-font-family, var(--font-family));
  line-height: var(--line-height, 1.5);
  letter-spacing: var(--letter-spacing, normal);
}

.shellui-home {
  box-sizing: border-box;
  min-height: 100svh;
  max-width: 40rem;
  margin: 0 auto;
  padding: 3rem 1.5rem;
  display: flex;
  flex-direction: column;
  justify-content: center;
  gap: 1rem;
  text-align: left;
}

.eyebrow {
  margin: 0;
  font-size: 0.85rem;
  font-weight: 600;
  letter-spacing: 0.08em;
  text-transform: uppercase;
  color: var(--primary, var(--foreground));
}

.shellui-home h1 {
  margin: 0;
  font-size: clamp(2rem, 4vw, 2.75rem);
  line-height: 1.15;
  letter-spacing: -0.03em;
  color: var(--foreground);
  font-family: var(--heading-font-family, var(--font-family, inherit));
}

.blurb {
  margin: 0;
  font-size: 1.05rem;
  line-height: 1.55;
  color: var(--muted-foreground, var(--foreground));
  opacity: 0.9;
}

.meta {
  margin: 0.75rem 0 0;
  padding: 1rem 0;
  border-block: 1px solid var(--border, color-mix(in oklab, var(--foreground) 12%, transparent));
  display: grid;
  gap: 0.85rem;
}

.meta div {
  display: grid;
  gap: 0.2rem;
}

.meta dt {
  font-size: 0.75rem;
  font-weight: 600;
  letter-spacing: 0.04em;
  text-transform: uppercase;
  color: var(--muted-foreground, var(--foreground));
  opacity: 0.75;
}

.meta dd {
  margin: 0;
  font-family: var(--font-family, ui-monospace, monospace);
  font-size: 0.95rem;
  color: var(--foreground);
}

.hint {
  margin: 0;
  font-size: 0.9rem;
  line-height: 1.45;
  color: var(--muted-foreground, var(--foreground));
  opacity: 0.8;
}
</style>

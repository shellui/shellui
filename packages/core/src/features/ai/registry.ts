...
export class AiRegistry {
  ...
  /**
   * Clear conversation state on the adapter that owns `modelId` (WebLLM hard reset).
   * `sessionId` (from `create`) becomes the adapter's new owning session.
   * No-op when the adapter does not implement resetConversation.
   */
  async resetConversation(modelId?: string | null, sessionId?: string): Promise<void> {
    if (!modelId) {
      // Reset every adapter that supports it (destroy without a clear model).
      await Promise.all(
        this.listAdapters().map(
          (adapter) => adapter.resetConversation?.(undefined, sessionId) ?? Promise.resolve(),
        ),
      );
      return;
    }
    ...
    if (!resolved) return;
    await resolved.adapter.resetConversation?.(resolved.modelId, sessionId);
  }
}

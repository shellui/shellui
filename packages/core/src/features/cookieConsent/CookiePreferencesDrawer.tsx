...
interface CookiePreferencesDrawerProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** When true, closing the drawer without saving will reject all cookies (for initial consent flow) */
...
export function CookiePreferencesDrawer({
  open,
  onOpenChange,
  isInitialConsent = false,
}: CookiePreferencesDrawerProps) {
  ...
  const handleOpenChange = useCallback(
    ...
    [currentAcceptedHosts, strictNecessaryHosts, isInitialConsent, allHosts, updateSetting, onOpenChange]
  );
  ...
  const handleAcceptAll = useCallback(() => {
    ...
    onOpenChange(false);
  }, [allHosts, updateSetting, onOpenChange]);
  ...
  const handleRejectAll = useCallback(() => {
    ...
    onOpenChange(false);
  }, [strictNecessaryHosts, allHosts, updateSetting, onOpenChange]);
  ...
  const handleSave = useCallback(() => {
    ...
    onOpenChange(false);
  }, [localAcceptedHosts, strictNecessaryHosts, allHosts, updateSetting, onOpenChange]);
  ...
  return (
    <Drawer open={open} onOpenChange={handleOpenChange} direction="right">
      <DrawerContent direction="right" size="420px" className="flex flex-col">
        <DrawerHeader>
          <DrawerTitle>{t('preferences.title')}</DrawerTitle>
          <DrawerDescription>{t('preferences.description')}</DrawerDescription>
        </DrawerHeader>

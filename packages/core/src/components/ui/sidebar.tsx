...
function clampSidebarWidth(widthPx: number): number {
  return Math.min(SIDEBAR_WIDTH_MAX_PX, Math.max(SIDEBAR_WIDTH_MIN_PX, Math.round(widthPx)));
}

function readSidebarWidth(fallbackPx: number): number {
...
function writeSidebarWidth(widthPx: number): void {
  if (typeof window === 'undefined') return;
  try {
    sessionStorage.setItem(SIDEBAR_WIDTH_STORAGE_KEY, String(clampSidebarWidth(widthPx)));
  } catch {
    // Ignore quota / privacy mode errors.
  }
}
...
type SidebarContextProps = {
  ...
  isMobile: boolean;
  toggleSidebar: () => void;
  /** Expanded desktop sidebar width in CSS pixels. */
  width: number;
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // Persist desktop expanded/collapsed across refresh for this tab session.
      writeSidebarOpen(openState);
    },
    [setOpenProp, open],
  );

  const setWidth = React.useCallback((next: number, options?: { persist?: boolean }) => {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          'fixed inset-y-0 z-10 hidden h-[var(--shellui-app-height,100dvh)] w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear group-data-[resizing]/sidebar-wrapper:transition-none md:flex',
          side === 'left'
      ...
      >
        <div
          ...
          className="flex h-full w-full flex-col bg-sidebar pt-[var(--shellui-safe-area-top)] pb-[var(--shellui-safe-area-bottom)] group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow-sm"
        >
          {children}
        </div>
      </div>
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLButtonElement>) => {
    ...
    if (!didDragRef.current && Math.abs(delta) < SIDEBAR_RESIZE_DRAG_THRESHOLD_PX) return;
    didDragRef.current = true;

    const nextWidth = drag.side === 'left' ? drag.startWidth + delta : drag.startWidth - delta;
    widthRef.current = clampSidebarWidth(nextWidth);
    setWidth(nextWidth, { persist: false });
  };
  ...
  const endDrag = (event: React.PointerEvent<HTMLButtonElement>) => {
    ...
    if (didDragRef.current) {
      setWidth(widthRef.current, { persist: true });
    }
    try {
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      style={{ fontFamily: 'var(--heading-font-family, inherit)', ...style }}
      {...props}
    />
  );
}

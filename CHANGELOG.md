...
- **Mobile bottom sidebar:** sidebar / sidebar-inset mobile nav opens as a bottom sheet with `sm` | `md` | `lg` snap sizes (`mobileSidebarSize` config + `SidebarProvider` / `useSidebar` API). Drag the grab handle to resize; drag past `sm` or tap the overlay to close; sheet slides up from below (respects `prefers-reduced-motion`).

...
Recent Chromium exposes a built-in on-device model through the **Prompt API** (`LanguageModel`, formerly `window.ai.languageModel`). When present, Shellui feature-detects it and shows a **Built-in browser model** row under **Settings → AI → Providers** with the hint “Built-in browser model (Chrome Prompt API)”. The provider **Enable / Disable** switch is the main control. If availability is `downloadable` (or a one-shot `create()` is still needed), an **Enable** button — or turning the switch on — warms the model via `create({ monitor })`. Chrome often reports no transferable download progress (model already on device, or `downloadprogress` never advances), so Settings shows indeterminate “Setting up built-in model…” rather than a stuck 0% Install bar; a real progress bar appears only when `downloadprogress` fires with `loaded` > 0. Once `available` / Ready, the switch alone is enough — there is no Install/Download wording for this provider (WebLLM catalog installs stay as real weight fetches). Apps then call the same `shellui.ai` path as any other provider.

...
If this release changes agent-relevant APIs or config (CLI, SDK, config schema), update the matching skill in [shellui/skills](https://github.com/shellui/skills), bump `metadata.version` and that skill’s `CHANGELOG.md`, and keep the skill lean (see [ADR 0001](/adr/ai-skill)).
